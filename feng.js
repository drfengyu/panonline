        ...
        function updateli(data) {
            ...
            data.forEach(file => {
                if (file.type == "file") {
                    ...
                    a.href = encodeURI(file.download_url);
                    a.innerText = decodeURIComponent(file.name);
                    li.appendChild(a);
        ...
        function FileLists() {
            ...
            var path = "upload";
			var apiUrl = "https://api.github.com/repos/" + username + "/" + repoName + "/contents/" + path + "/";
			console.log("start update...");
			fetch(apiUrl)
				.then(response => response.json())
				.then(data => { updateli(data); console.log(data); })
		...
		function encode64(input) {
			var output = ""; var chr1, chr2, chr3 = ""; var enc1, enc2, enc3, enc4 = ""; var i = 0; do { chr1 = input.charCodeAt(i++); chr2 = input.charCodeAt(i++); chr3 = input.charCodeAt(i++); enc1 = chr1 >> 2; enc2 = ((chr1 & 3) << 4) | (chr2 >> 4); enc3 = ((chr2 & 15) << 2) | (chr3 >> 6); enc4 = chr3 & 63; if (isNaN(chr2)) { enc3 = enc4 = 64; } else if (isNaN(chr3)) { enc4 = 64; } output = output + keyStr.charAt(enc1) + keyStr.charAt(enc2) + keyStr.charAt(enc3) + keyStr.charAt(enc4); chr1 = chr2 = chr3 = ""; enc1 = enc2 = enc3 = enc4 = ""; } while (i < input.length);
			return output;
		...
		function utf8to16(str) {
			var out, i, len, c; var char2, char3; out = "";
			len = str.length; i = 0;
			while (i < len) {
		...
		function uploadFile() {
			...
			reader.onload = function(event) {
                ...
                var path = "upload/" + encodeURIComponent(filename);
                ...
                var token ="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";
				var data = {
					message: message,
					content: encode64(utf16to8(content))
				};
